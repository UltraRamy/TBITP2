...
class BSBIIndex:
    ...
    def retrieve_bm25(self, query, k=10, k1=1.2, b=0.75):
        ...
        Formula BM25 (Okapi BM25):
          IDF(t)       = log((N - df(t) + 0.5) / (df(t) + 0.5) + 1)
          TF_norm(t,D) = tf(t,D) * (k1+1) / (tf(t,D) + k1*(1 - b + b*|D|/avgdl))
          score(D,Q)   = sum over query terms of IDF(t) * TF_norm(t,D)

        avg_doc_length sudah di-precompute saat indexing dan disimpan di index metadata.

        Parameters
        ----------
        query: str
            Query tokens yang dipisahkan spasi
        k: int
        ...
        ------
        List[(float, str)]
            List of (score, doc_name), terurut menurun berdasarkan skor
        """
        if len(self.term_id_map) == 0 or len(self.doc_id_map) == 0:
            self.load()
    ...
    def retrieve_bm25_wand(self, query, k=10, k1=1.2, b=0.75):
        ...
        WAND menggunakan upper bound skor per term untuk skip dokumen yang
        tidak mungkin masuk ke top-K, sehingga tidak semua dokumen perlu
        dihitung skor BM25-nya secara penuh.

        Upper bound per term (UB):
          UB(t) = IDF(t) * max_tf(t) * (k1+1) / (max_tf(t) + k1*(1-b))
        Asumsi: panjang dokumen = 0 (skenario terburuk) untuk mendapat upper bound yang valid.

        Algoritma WAND:
          1. Urutkan term berdasarkan current doc ID (cursor masing-masing term)
          2. Cari 'pivot': term pertama dimana akumulasi UB >= threshold (skor minimum top-K)
          3. Jika semua cursor sebelum pivot menunjuk ke pivot doc -> score dokumen itu penuh
          4. Jika tidak -> advance semua cursor sebelum pivot ke pivot doc
          5. Ulangi sampai semua postings habis
        ...
        with InvertedIndexReader(self.index_name, self.postings_encoding, directory=self.output_dir) as merged_index:
            ...
            for term in terms:
                ...
                postings, tf_list = merged_index.get_postings_list(term)
                term_data.append([ub, idf, postings, tf_list, 0])
            ...
            while True:
                term_data = [td for td in term_data if td[4] < len(td[2])]
                if not term_data:
                    break

                term_data.sort(key=lambda td: td[2][td[4]])
                ...
                pivot_doc = term_data[pivot_idx][2][term_data[pivot_idx][4]]
                first_doc = term_data[0][2][term_data[0][4]]

                if first_doc == pivot_doc:
                    score = 0.0
                    for td in term_data:
                        cursor = td[4]
                        if cursor < len(td[2]) and td[2][cursor] == pivot_doc:
                            tf = td[3][cursor]
                    ...
                    if len(top_k) < k:
                        heapq.heappush(top_k, (score, pivot_doc))
                        threshold = top_k[0][0] if len(top_k) == k else 0.0
                    elif score > top_k[0][0]:
                        heapq.heapreplace(top_k, (score, pivot_doc))
                        threshold = top_k[0][0]
                else:
                    for td in term_data[:pivot_idx]:
                        cursor = td[4]
                        while cursor < len(td[2]) and td[2][cursor] < pivot_doc:
                            cursor += 1
                        td[4] = cursor

            result = [(score, self.doc_id_map[doc_id]) for score, doc_id in top_k]
